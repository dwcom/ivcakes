										...
										foreach ( $attributes as $attribute_name => $options ) : ?>
                                            <div class="info__variable">
                                                ...
                                                <select id="<?php echo esc_attr( $attribute_name ); ?>"
                                                        class="variable-attribute fs20 getSelect"
...
<section class="special">
    ...
    <script>
	    function productAddNotion(productName) {
		    ...
		    $('#notionBlock').append(notification);
	    ...
	    jQuery(document).ready(function ($) {
		    ...
		    // Обработка клика по кнопке добавления вариативного товара в корзину
		    ...
		    // Инициализация Select2 для выбора вариативных атрибутов
		    $('.variable-attribute').select2().on('select2:select', function () {
			    updatePrice();
			    checkAndSetDescription();
		    });
		    
		    // Функция обновления цены при смене вариативных атрибутов
		    function updatePrice() {
			    const $productOptions = $('.variable-product-options');
			    const variations = $productOptions.data('variations');
			    const currencySymbol = $productOptions.data('currency-symbol');
			    ...
			    if (allSelected) {
				    ...
				    if (matchedVariation && matchedVariation.display_price) {
					    $priceElement.text(parseFloat(matchedVariation.display_price).toFixed(2) + ' ' + currencySymbol);
				    } else {
		    ...
		    // Функция обновления описания при смене вариативных атрибутов
		    function checkAndSetDescription() {
			    let selectedOption = $('.variable-attribute').val();
			    let variationsData = {};
			    ...
			    $.ajax({
				    ...
				    success: function (response) {
					    ...
					    if (selectedOption && variationsData[selectedOption]) {
						    $('#subtitleEditor').html(variationsData[selectedOption]);
					    } else {
						    $('#subtitleEditor').html($('#defaultProductContent').html());
					    }
				    },
				    error: function () {
