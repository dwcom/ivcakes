		...
		jQuery(document).ready(function ($) {
			...
			jQuery(document).ready(function ($) {
				$('#addVariableToCartButton').on('click', function () {
					...
					// Проверяем, определён ли AJAX URL WooCommerce
					if (typeof woocommerce_params === 'undefined' || !woocommerce_params.ajax_url) {
						console.error('Ошибка: woocommerce_params.ajax_url не определён.');
						alert('Ошибка конфигурации. Попробуйте позже.');
						return;
					}
			...
			jQuery(document).ready(function ($) {
				...
				// Подключаем Select2, если он ещё не инициализирован
				$('.variable-attribute').select2();
				
				// Добавляем обработчик событий Select2
				$('.variable-attribute').on('select2:select', updatePrice);
				
				// Вызываем updatePrice при загрузке страницы, если есть предвыбранные значения
				$(window).on('load', updatePrice);
			});
			...
			jQuery(document).ready(function ($) {
				...
				function checkAndSetDescription() {
					let selectedOption = $('.variable-attribute').val(); // Получаем выбранное значение
				...
				// Подключаем обработчик для Select2
